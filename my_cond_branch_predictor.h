...
#ifndef NUM_BHT_INDEX_BITS
#define NUM_BHT_INDEX_BITS 16
#endif
...
#define BHT_SIZE (1 << NUM_BHT_INDEX_BITS)
...
class MyPredictor {
    private:
    std::array<uint8_t, BHT_SIZE> bht;
    branch_hist_t gbh;
    ...
    int get_index(uint64_t pc, branch_hist_t bh) {
        // (PC mod BHT size) XOR (Left-aligned GBH)
        return ((pc >> 2) & BHT_INDEX_MASK) ^ (bh << (NUM_BHT_INDEX_BITS - BRANCH_HISTORY_LENGTH));
    }
    ...
    void setup() {
        // Check # of bits used
        int bht_count = BHT_SIZE * NUM_SATURATION_BITS;
    ...
    void update_on_unconditional() {
#ifdef UPDATE_ON_UNCONDITIONAL
        update_hist(gbh, true);
#endif
    }
    ...
    void update(uint64_t seq_no, uint8_t piece, uint64_t pc, bool taken) {
        ...
        int bht_index = get_index(pc, br_hist);
